...
opt = open('orca_s1_freq.in').readlines()
sh = open('orca_freq.sh').readlines()
...
lists = os.listdir('.')
#lists = [d for d in lists if '21' not in d and '19' not in d]
#lists = ['1']

for par in lists:
    if os.path.isdir(par) and par.isdigit():
        directory = par+'/best_geom/opt_b97-3c/s1opt'
        os.chdir(directory)
        ...
        title = ['#!/bin/bash\n#SBATCH --job-name=' + prefix + str(lowest_number) +'-'+ str(biggest_number) +'_1FREQ\n']
        new_sh = title + sh
