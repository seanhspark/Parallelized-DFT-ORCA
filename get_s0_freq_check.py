        ...
        for filename in sorted(os.listdir(subdir_path)):
            if filename.startswith("slurm-"):
                ...
                if extract_frequency_data(os.path.join(subdir_path, filename)):
                    count_true += 1
                    subdir_has_imaginary = True
                    
        if subdir_has_imaginary:
            subdirs_with_imaginary.append(str(subdir))
                    
        if count_subdir != 5:
            logging.info(f"subdirectory {subdir} does not have '5' output files.")

    return count_total, count_true, subdirs_with_imaginary


if __name__ == "__main__":
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_filename = f"s0_b97-3c_freq_check_log_{current_time}.txt"
    ...
    count_total, count_true, subdirs_with_imaginary = main()
    
    logging.info("\nTotal number of files processed: " + str(count_total))
    logging.info("Number of files with imaginary frequencies: " + str(count_true))
    logging.info(f"Subdirectories with imaginary frequencies: {subdirs_with_imaginary}")
    logging.info("\nDONE")